...
class GCNWithMultiHeadGATAndTCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nout, mat_path, dropout=0.3, num_heads=4):
        ...
        self.gc2 = GraphConvolution(nfeat, nhid, mat_path)
        self.gc3 = GraphConvolution(nfeat, nhid, mat_path)
    ...
    def forward(self, x, adj, epoch=0, max_epochs=100):
        ...
        # 第二层 GCN
        x = self.gc2(x, epoch=epoch, max_epochs=max_epochs)
        x = self.bn2(x.transpose(1, 2)).transpose(1, 2)
        x = F.relu(x)

        # 第三层 GCN
        x = self.gc3(x, epoch=epoch, max_epochs=max_epochs)
