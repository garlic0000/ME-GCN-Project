...
def compare_label_overall(all_npz_root_path1, all_npz_root_path2):
    ...
    if len(all_npz_path_list1) != len(all_npz_path_list2):
        logging.info("两个npz文件地址列表长度不同")
        logging.info(f"{all_npz_root_path1}: {len(all_npz_path_list1)}个")
        logging.info(f"{all_npz_root_path2}: {len(all_npz_path_list2)}个")
        return
    for npz_file_path1, npz_file_path2 in zip(all_npz_path_list1, all_npz_path_list2):
        ...
        if video_name1 != video_name2:
            logging.info("不是同一段视频")
            logging.info(f"npz1: {npz_file_name1}")
            logging.info(f"npz2: {npz_file_name2}")
            logging.info("")
            continue
        if label1.shape != label2.shape:
            logging.info("两个标签矩阵尺寸不同")
            logging.info(f"npz1: {npz_file_name1} size: {label1.shape}")
            logging.info(f"npz2: {npz_file_name2} size: {label2.shape}")
            logging.info("")
            continue
        if np.array_equal(label1, label2):
            logging.info("两个标签矩阵内容完全相同")
            logging.info(f"npz1: {npz_file_name1} size:{label1.shape}")
            logging.info(f"npz2: {npz_file_name2} size:{label2.shape}")
            logging.info("")
        else:
            logging.info("两个标签矩阵内容不完全相同")
            logging.info(f"npz1: {npz_file_name1} size:{label1.shape}")
            logging.info(f"npz2: {npz_file_name2} size:{label2.shape}")
            logging.info("")
...
def output_all_label(not_all_same_micro_label_list, all_same_micro_label_list, not_all_same_macro_label_list,
    ...
    logging.info(f"微表情分段标签 不完全相同的 总共{len(not_all_same_micro_label_list)}对")
    if len(not_all_same_micro_label_list):
        for item in not_all_same_micro_label_list:
            ...
            npz_file_name2, micro_start_indices2, micro_apex_indices2, micro_end_indices2, micro_action_indices2 = item[
                1]
            logging.info("*******************************************************")
            logging.info(f"分段标签 {npz_file_name1}")
            logging.info(f"起始帧: {micro_start_indices1}    顶点帧: {micro_apex_indices1}    偏移帧: {micro_end_indices1}")
            logging.info(f"微表情区间: {micro_action_indices1}")
            logging.info(f"分段标签 {npz_file_name2}")
            logging.info(f"起始帧: {micro_start_indices2}    顶点帧: {micro_apex_indices2}    偏移帧: {micro_end_indices2}")
            logging.info(f"微表情区间: {micro_action_indices2}")
            logging.info("")  # 输出一个换行
    logging.info("-----------------------------------------------------------------------------------------------------------")

    logging.info(f"微表情分段标签 完全相同的 总共{len(all_same_micro_label_list)}对")
    if len(all_same_micro_label_list):
        for item in all_same_micro_label_list:
            ...
            logging.info(f"分段标签 {npz_file_name}")
            logging.info(f"起始帧: {micro_start_indices}    顶点帧: {micro_apex_indices}    偏移帧: {micro_end_indices}")
            logging.info(f"微表情区间: {micro_action_indices}")
            logging.info("")  # 输出一个换行
    logging.info("-----------------------------------------------------------------------------------------------------------")

    logging.info(f"宏表情分段标签 不完全相同的 总共{len(not_all_same_macro_label_list)}对")
    if len(not_all_same_macro_label_list):
        for item in not_all_same_macro_label_list:
            ...
            logging.info("*******************************************************")
            logging.info(f"分段标签 {npz_file_name1}")
            logging.info(f"起始帧: {macro_start_indices1}    顶点帧: {macro_apex_indices1}    偏移帧: {macro_end_indices1}")
            logging.info(f"宏表情区间: {macro_action_indices1}")
            logging.info(f"分段标签 {npz_file_name2}")
            logging.info(f"起始帧: {macro_start_indices2}    顶点帧: {macro_apex_indices2}    偏移帧: {macro_end_indices2}")
            logging.info(f"宏表情区间: {macro_action_indices2}")
            logging.info("")  # 输出一个换行
    logging.info("-----------------------------------------------------------------------------------------------------------")

    logging.info(f"宏表情分段标签 完全相同的 总共{len(all_same_macro_label_list)}对")
    if len(all_same_macro_label_list):
        ...
        for item in all_same_macro_label_list:
            ...
            logging.info(f"分段标签 {npz_file_name}")
            logging.info(f"起始帧: {macro_start_indices}    顶点帧: {macro_apex_indices}    偏移帧: {macro_end_indices}")
            logging.info(f"宏表情区间: {macro_action_indices}")
            logging.info("")  # 输出一个换行
    logging.info("-----------------------------------------------------------------------------------------------------------")
...
if __name__ == "__main__":
    # 设置日志文件路径
    log_file_path = "./compare_label_logfile.log"
