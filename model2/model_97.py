...
class TCNBlock(nn.Module):
    """
    TCN layer with ResidualWeight Optimization, Time Positional Encoding, and Dilated Convolution
    """

    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, dropout=0.2, max_frames=30):
        super(TCNBlock, self).__init__()

        # 可学习的时间位置编码矩阵 E_tcn
        self.time_positional_encoding = nn.Parameter(torch.randn(max_frames, in_channels))
    ...
    def forward(self, x):
        ...
        # 获取时间位置编码矩阵并与空间特征拼接
        E_tcn = self.time_positional_encoding[:t, :].unsqueeze(0).repeat(b, 1, 1)  # [B, T, C]
        x = x.transpose(1, 2)  # [B, C, T, N]

        # 逐帧拼接时空特征
        H_gat = x.reshape(b, c, t * n)  # [B, C, T*N]
        H_tcn = torch.cat((H_gat, E_tcn), dim=1)  # [B, 2C, T*N]
...
class GCNWithMultiHeadGATAndTCN(nn.Module):
    """
    Modified GCN with Multi-Head GAT and a single TCN block with Time Positional Encoding
    """

    def __init__(self, nfeat, nhid, nout, mat_path, dropout=0.3, num_heads=4, max_frames=30):
        ...
        self.tcn1 = TCNBlock(nout, nout, kernel_size=3, dilation=1, dropout=0.2, max_frames=max_frames)
...
class AUwGCNWithMultiHeadGATAndTCN(torch.nn.Module):
    """
    AU detection model with GCN, Multi-Head GAT, and one TCN layer with Time Positional Encoding
    """
