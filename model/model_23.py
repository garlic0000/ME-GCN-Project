...
增加 ResidualWeight 类，用于动态调整主分支和残差分支的比例。
该模块可以在模型的每一层（如 GCN、GAT 和 TCN）后添加，提升残差的适应能力。
残差优化应用：

修改 GCN 层、GAT 层和 TCN 层的前向传播逻辑，增加残差连接优化。
...
class MultiHeadGraphAttentionLayer(nn.Module):
    ...
    def __init__(self, in_features, out_features, num_heads=4, dropout=0.6, alpha=0.2, drop_prob=0.1):
        ...
        self.a = nn.ModuleList([nn.Parameter(torch.zeros(1, self.out_per_head * 2)) for _ in range(num_heads)])  # Attention weights
