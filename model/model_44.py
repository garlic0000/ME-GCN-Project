...
2.基于层的特点动态设置
网络中有多层 ResidualWeight 模块，可以为不同的层设置不同的初始值
self.alpha = nn.Parameter(torch.tensor(np.random.uniform(0.3, 0.7)))  # 随机初始化在 [0.3, 0.7]

3.使用正态分布初始化
利用正态分布（或其他分布）对 alpha 进行初始化
self.alpha = nn.Parameter(torch.tensor(np.random.normal(0.5, 0.1)))  # 均值 0.5，标准差 0.1

4.训练时动态调整初始值
在训练的初始阶段，根据某些统计信息（如 input 和 residual 的分布）来动态调整初值
with torch.no_grad():
    initial_alpha = input.norm() / (input.norm() + residual.norm())
    self.alpha.data = torch.tensor(initial_alpha)
