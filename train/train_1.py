...
from model.model_29 import AUwGCNWithMultiHeadGATAndTCN
...
def train(opt, data_loader, model, optimizer, epoch, device, writer):
    ...
    reg_loss_weight = opt.get('reg_loss_weight', 1e-3)  # 正则化损失权重
    ...
    for batch_idx, (feature, micro_apex_score, macro_apex_score,
        ...
        # 获取模型输出和正则化损失
        output_probability, reg_loss = model(feature)
        # # 调试
        ...
        loss = (1.8 * loss_micro_apex
                ...
                + reg_loss_weight * reg_loss  # 正则化损失
                )
